...
AUMGDemoCharacter::AUMGDemoCharacter()
{
	...
	// Weapon variables default values
	WeaponCount = 0;
	CurrentWeapon = 0;
	MaxWeapons = 2;
...
void AUMGDemoCharacter::BeginPlay()
{
	...
	UMGDemoPlayerController = Cast<AUMGDemoPlayerController>(UGameplayStatics::GetPlayerController(GetWorld(), 0));
...
void AUMGDemoCharacter::AddWeapon(AWeaponBase* Weapon)
{
	...
	if (WeaponCount <= MaxWeapons)
	{
		...
		UMGDemoPlayerController->ShowWeaponWidgets(WeaponCount, Weapon);
