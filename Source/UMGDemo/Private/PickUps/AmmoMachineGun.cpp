        ...
        if (OtherActor->ActorHasTag("Player"))
        {
            ...
            for (int i = 0; i < UMGDemoCharacter->Weapons.Num(); i++)
            {
                if (UMGDemoCharacter->Weapons[i]->WeaponStruct.WeaponType == EWeaponType::EMachineGun)
                {
                    ...
                    {
                        UMGDemoCharacter->Weapons[i]->WeaponStruct.SetCurrentAmmo(UMGDemoCharacter->Weapons[i]->WeaponStruct.GetCurrentAmmo() + this->PickUpStruct.AmmoAmount);
